...
def create_tfrecord_dataset(
    ...
    cache_buffer_size,       # 8192 dataset elements will be buffered to tune the uniformity of shuffling
    ...
    def _parse(example_proto):
        example = tf.io.parse_single_example(example_proto, feature_description) # Parse the input `tf.train.Example` proto using the defined dictionary above
        input, target = example["input"], example["target"]
        return (input, target)

    dataset = dataset.map(_parse)
    dataset = dataset.cache().shuffle(cache_buffer_size).batch(batch_size)       # caches the dataset to memory in chunks and shuffle the samples in internal buffer
    dataset = dataset.prefetch(buffer_size=prefetch_buffer_size)                 # number of batches to asynchronously to load for better throughput
    return dataset